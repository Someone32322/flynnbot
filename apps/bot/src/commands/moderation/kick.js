...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!target) {
      await interaction.reply(ephemeral("Usage: /kick <user> [reason]"));
      return;
    }
    ...
    if (!(await checkImmuneRoles(interaction, target.targetMember, "kick", modConfig))) {
      return;
    }

    const reason = interaction.options.getString("reason") || "No reason provided.";
    const dmResult = shouldSendDm(modConfig, "onPunish")
      ? await sendDmNotice(target.targetUser, buildActionDm("kick", interaction.guild.name, interaction.user.tag, reason))
