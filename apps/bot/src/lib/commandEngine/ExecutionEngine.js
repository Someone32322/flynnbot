 ...
 * ExecutionEngine
 *
 * The core command runtime. Handles:
 *   - Command lookup and caching
 *   - Condition checking (roles, channels, permissions, cooldowns)
 *   - Block execution with depth/timeout/loop guards
 *   - Stored variable persistence after execution
 *   - Execution metadata recording
 *
 * Phase 1: This is a clean foundation stub.
 * Phase 2: Full implementation of all methods.
 */
class ExecutionEngine {
  /** @param {import('discord.js').Client} client */
  constructor(client) {
    ...
    this._cacheTTL = 5_000; // 5 seconds — matches dashboard save latency
  }

  // ── Block Registry ────────────────────────────────────────────

  /**
   * Register block executors. Called at startup.
   * @param {function(BlockRegistry): void} loader
   */
  loadBlocks(loader) {
    loader(this.registry);
  }
  ...
  /**
   * Invalidate cache for a guild. Called after dashboard saves.
   ...
   * Defers reply BEFORE any async work.
   *
  ...
  /**
   * Handle a button or select menu component interaction.
   *
  ...
  /**
   * Handle a message for text-based triggers (prefix, contains, exact, startsWith, regex).
   *
  ...
  /**
   * Handle a voice state update (join / leave).
   *
  ...
  /**
   * Handle the scheduled tick for interval-based commands.
   ...
   * This is the central method that all trigger handlers call.
   *
   * @param {object} cmd - GuildCommand document
   * @param {ExecutionContext} ctx
   * @returns {Promise<{ status: 'success'|'error'|'partial', durationMs: number }>}
   */
  async execute(cmd, ctx) {
    const startedAt = Date.now();
    let status = 'success';
    ...
    try {
      ...
      // TODO: Phase 2 — check conditions (roles, channels, cooldowns)
      // TODO: Phase 2 — execute blocks
      // TODO: Phase 2 — persist dirty stored variables

      console.log(`[CommandEngine] Stub execute called for cmd=${cmd.name} guild=${ctx.guildId}`);
    } catch (err) {
      console.error(`[CommandEngine] Execution error cmd=${cmd._id}:`, err);
      status = 'error';
    }
  ...
  /**
   * Build an ExecutionContext from a Discord event.
   *
   * @param {object} opts
   * @returns {Promise<ExecutionContext>}
   */
  async buildContext({ guild, member, channel, interaction, message, trigger, storedVars = {} }) {
    return new ExecutionContext({
      client:      this.client,
      guild,
      member,
